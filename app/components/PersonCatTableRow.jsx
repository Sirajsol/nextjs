...
const PersonCatTableRow = ({cat,setCat,setEShow,setMutate}) => {
    ...
    const[deleting,setDeleting]=useState(false)
        const del=async()=>{
          setDeleting(true)
            ...
            const s=await fetch(`/api/perrsoncat?id=${cat.id}`,{
                method:'DELETE'
            }).then(res=>{
                if (res.status=='200'){
                  setDeleting(false)
...
<div className=" flex flex-1 border-[1px] justify-end items-center bg-blue-950 text-yellow-500  text-[20px]  whitespace-nowrap  overflow-hidden "><div className=" mr-[5px]  justify-end  hover:overflow-scroll">{cat.id}</div></div>
...
<div className=" flex flex-1  justify-end items-center bg-blue-950 text-yellow-500  text-[20px]  whitespace-nowrap  overflow-hidden "><div className={` justify-end   hover:overflow-scroll`}>{cat?.title}</div></div>
...
<div className=" flex flex-1 border-[1px] justify-end items-center bg-blue-950 text-yellow-500  text-[20px]  whitespace-nowrap  overflow-hidden "><div style={{color:`${cat.color}`}} className=" mr-[5px]  justify-end  hover:overflow-scroll">{cat.color}</div></div>
...
export default PersonCatTableRow;

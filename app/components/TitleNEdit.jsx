...
const TitleNEdit = ({tit,setEShow,setMutate}) => {
    ...
    const postTl =async()=>{
        ...
        setEditing(true)
const tt=await fetch(`/api/titleone/${tit.id}`,{
    method:'PUT',
    body:JSON.stringify({title,lang})
}).then(res=>{
    setEditing(false)
    if(res.status=='200'){
        toast.success("تم تعديل لقب أول")
        setMutate(true) 
    }
...
setEShow(false)
    ...
    console.log('eroorrrrrrrrrrrrrrrrrrrrrrrrrrrrrr ')
    alert(error)
})
    }
useEffect(()=>{
